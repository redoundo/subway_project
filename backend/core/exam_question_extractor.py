...
def extract_questions_options_and_positions(pdf_path):
    """
    PDF 파일을 열고, 각 페이지의 텍스트와 위치를 추출하여 시험 문항 번호, 선택지 텍스트, 그리고 선택지의 위치(rect)를 가져옵니다.
    선택지는 ①, ② 등으로 시작하는 줄을 찾고, 그 위치를 기록합니다.
    """
    doc = fitz.open(pdf_path)
    questions = {}  # {question_number: [{'text': option_text, 'rect': (x0, y0, x1, y1)}]}

    # 선택지 패턴
    option_pattern = re.compile(r'^([①②③④⑤])\s*(.*)')
    question_pattern = re.compile(r'^(\d+)\.\s')  # 문항 번호 패턴

    current_question = None

    for page_num in range(doc.page_count):
        page = doc.load_page(page_num)
        blocks = page.get_text("blocks")  # 텍스트 블록 추출: 각 블록은 (x0, y0, x1, y1, "text", block_no, block_type)
        for block in blocks:
            if block[6] != 0:  # block_type 0이 텍스트 블록
                continue
            text = block[4].strip()
            lines = text.split('\n')
            for line in lines:
                ...
                if o_match and current_question:
                    option_num = o_match.group(1)
                    option_text = o_match.group(2).strip()
                    optim_content = o_match.group(1)
                    optim_instances = page.search_for(optim_content)
                    option_rect = None
                    if optim_instances:
                        for inst in optim_instances:
                            if (block[0] > inst.x0) or (inst.y0 < block[1]) or (inst.x1 > block[2]) or (inst.y1 > block[3]):
                                continue
                            # inst는 Rect 객체이며 (x0, y0, x1, y1) 형태의 좌표를 가집니다.
                            transformed_rect = inst * html_mat
                            # x0, y0는 텍스트를 감싸는 사각형의 왼쪽 상단 좌표입니다.
                            x0, y0, x1, y1 = map(int, transformed_rect)
                            option_rect = (x0, y0, x1, y1)
                    # 위치는 블록의 rect 사용 (정확히는 줄 단위가 아니지만, 근사치로 블록 위치 사용)
                    rect = (block[0], block[1], block[2], block[3]) if option_rect is None else option_rect
